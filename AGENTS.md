# AGENTS.md — AI Context File for RTB Static-Inference Engine (Self-Loop Deferred)

> **Purpose:** Paste this file at the start of any AI conversation (Codex, GPT, Gemini, etc.)
...
**CVR Model Decision — PENDING aggregation of all 6 conv.txt files.**

Competition scoring: Score = Clicks + N × Conversions
N per advertiser: 1458→0, 3358→2, 3386→0, 3427→0, 3476→10

conv_06.txt alone: 43 conversions, ALL advertiser 3358. Too sparse.
Decision rule:
- If 6-day total < few hundred OR concentrated in 1-2 advertisers:
    Drop CVR model. Use CTR-only + advertiser multipliers.
- If 6-day total in thousands spread across advertisers:
    Add CVR model: bid = CTR×(clickVal + N×CVR×convVal)×1000
ACTION: upload remaining 5 conv.txt files next session and aggregate.

---

**Bidding Architecture — locked in (pending CVR decision above).**

Pipeline order (reject-early principle, senior eng approved):
```
BidRequest
    ↓
...
FTRLModel::predict()  — dot product + sigmoid
    ↓
FloorGate             — REJECT EARLY: bid < floor → no bid
    ↓
BudgetManager         — REJECT EARLY: budget exhausted → no bid
    ↓
BidCalculator         — only runs if we are actually bidding
    ↓
BidResponse
```

FloorGate and BudgetManager before BidCalculator deliberately.
Don't compute the bid price if you are not going to bid.

Bid formula v1 (CTR-only):
  bid = predicted_ctr × advertiser_multiplier × 1000
...
Floor price: owned by FloorGate only. Single responsibility.

---

**Progress estimate (end of this session):**
- Done: BidRequest, FeatureExtractor (v2 with Feature struct), ArenaAllocator,
  FTRLModel (hpp+cpp)
- Dropped: FlatHashMap, CVR model (pending data check)
- Remaining C++: FloorGate, BudgetManager, BidCalculator, main loop wiring
- Remaining tests: rewrite test_feature_extractor.cpp, write test_ftrl_model.cpp
- Remaining Python: feature engineering, FTRL trainer (Algorithm 1),
  weight serializer, progressive validation
- Estimate: ~6-8 more sessions to working v1

**Next session starts with:**
1. Upload remaining 5 conv.txt files → aggregate → lock CVR decision
2. Rewrite test_feature_extractor.cpp for Feature{bucket,value} design
3. Write test_ftrl_model.cpp
4. Then: FloorGate (simplest remaining component)
